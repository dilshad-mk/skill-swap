...
function Step2Skills({ data, onUpdate }) {
    ...
    const handleSubSkillClick = (sub) => {
        ...
        // ✅ FIX: send updated state
        onUpdate({ teach: updatedSkills });
    }

    return (
        <div className='flex flex-col bg-[white] px-5 py-5 mx-3 items-center max-w-[600px] md:min-w-150 sm:min-w-100 rounded-2xl shadow-xl'>
                ...
                <div className='w-full mt-4'>
                    ...
                    <div className='flex flex-wrap gap-2 mt-2'>
                        {selectedSubskills.map((item, index) => (
                            <div key={index} className='bg-blue-500 text-white px-3 py-1 rounded-full text-[10px]'>
                                {item}
                            </div>
                        ))}
                    </div>
                ...
                <div className='grid md:grid-cols-3 sm:grid-cols-2 md:grid-rows-2 gap-2 grid-cols-2 py-1'>
                    {skilldata.map((skills) => (
                        <div key={skills.id} onClick={() => handleskillclick(skills)}
                            ...
                            ${slectedSkill?.id === skills.id ? "bg-blue-100 border-blue-800 border-2" : "border"}`}>
                            <i className={`${skills.logo} text-2xl ${slectedSkill?.id === skills.id ? "text-blue-800" : "text-black"}`}></i>
                            ...
                            <div className="flex flex-wrap gap-[8px] mt-2">
                                {slectedSkill.subskill.map((sub, index) => (
                                    <div key={index}
                                        ...
                                        className={`border text-[10px] py-1 px-1 rounded-[4px] w-[120px] cursor-pointer
                                        ${selectedSubskills.includes(sub) ? "bg-blue-700 text-white" : "bg-white"}`}>
                                        {sub}
                                    </div>
                                ))}
                            </div>
